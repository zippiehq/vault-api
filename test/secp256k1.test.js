...
describe('Secp256k1', function() {
  ...
  describe('Key Info Fetch Tests', function() {
      ["m/0","m/1","m/1/1"].forEach(function(key) {
       it('keyinfo', function(done) {
        vaultsecp256k1.keyInfo(vault, key).then(keyInfo => {
  ...
  describe('Sign Tests', function() {
    it('sign', function(done) {
      vaultsecp256k1.sign(vault, "m/0", shajs('sha256').update('data to sign goes here').digest())
        .then(output => {
  ...
  describe('Encrypt / Decrypt Tests', function() {
    it('encrypt', function(done) {
      vaultsecp256k1.keyInfo(vault, "m/0").then((keyInfo) => {
        vaultsecp256k1.encrypt(vault, keyInfo.pubkey, Buffer.from("message to encrypt").toString('hex')).then(message => {
    ...
    it('decrypt', function(done) {
      vaultsecp256k1.keyInfo(vault, "m/0").then((keyInfo) => {
        vaultsecp256k1.encrypt(vault, keyInfo.pubkey, Buffer.from("message to encrypt").toString('hex')).then(message => {
          vaultsecp256k1.decrypt(vault, "m/0", message).then(decrypted => {
