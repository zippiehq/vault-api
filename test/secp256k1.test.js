...
describe('Secp256k1', function() {
  ...
  describe('Key Info Fetch Tests', function() {
      ["m/0","m/1","m/1/1"].forEach(function(key) {
       it('keyinfo', function(done) {
        vault.secp256k1.keyInfo(key).then(keyInfo => {
  ...
  describe('Encrypt / Decrypt Tests', function() {
    it('encrypt', function(done) {
      vault.secp256k1.keyInfo("m/0").then((keyInfo) => {
        vault.secp256k1.encrypt(keyInfo.pubkey, Buffer.from("message to encrypt").toString('hex')).then(message => {
    ...
    it('decrypt', function(done) {
      vault.secp256k1.keyInfo("m/0").then((keyInfo) => {
        vault.secp256k1.encrypt(keyInfo.pubkey, Buffer.from("message to encrypt").toString('hex')).then(message => {
          vault.secp256k1.decrypt("m/0", message).then(decrypted => {
